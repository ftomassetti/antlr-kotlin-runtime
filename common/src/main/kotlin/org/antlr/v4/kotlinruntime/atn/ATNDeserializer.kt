 ...
 * @author Sam Harwell
 */
class ATNDeserializer @JvmOverloads constructor(deserializationOptions: ATNDeserializationOptions? = ATNDeserializationOptions.defaultOptions) {
    ...
    fun deserialize(data: CharArray): ATN {
        ...
        if (version != SERIALIZED_VERSION) {
            val reason = String.format(Locale.getDefault(), "Could not deserialize ATN with version %d (expected %d).", version, SERIALIZED_VERSION)
        ...
        if (!SUPPORTED_UUIDS.contains(uuid)) {
            val reason = String.format(Locale.getDefault(), "Could not deserialize ATN with UUID %s (expected %s or a legacy UUID).", uuid, SERIALIZED_UUID)
            throw UnsupportedOperationException(InvalidClassException(ATN::class.java!!.getName(), reason))
        }
            ...
            if (supportsLexerActions) {
                ...
                for (state in atn.states) {
                    for (i in 0 until state.numberOfTransitions) {
                        ...
                        val ruleIndex = transition.ruleIndex
                        val actionIndex = transition.actionIndex
    ...
    fun stateFactory(type: Int, ruleIndex: Int): ATNState? {
        ...
        when (type) {
            ...
            ATNState.LOOP_END -> s = LoopEndState()
                ...
                val message = String.format(Locale.getDefault(), "The specified state type %d is not valid.", type)
    ...
    protected fun lexerActionFactory(type: LexerActionType, data1: Int, data2: Int): LexerAction {
        when (type) {
            ...
            LexerActionType.TYPE -> return LexerTypeAction(data1)
                ...
                val message = String.format(Locale.getDefault(), "The specified lexer action type %d is not valid.", type)
                throw IllegalArgumentException(message)
            }
        }
    }
    ...
    companion object {
        ...
        fun toLong(data: CharArray, offset: Int): Long {
            val lowOrder = toInt32(data, offset) and 0x00000000FFFFFFFFL
