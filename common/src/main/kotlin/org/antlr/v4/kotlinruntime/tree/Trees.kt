...
object Trees {
    ...
    fun toStringTree(t: Tree, recog: Parser?): String {
        val ruleNames = if (recog != null) recog!!.getRuleNames() else null
    ...
    fun getNodeText(t: Tree, ruleNames: List<String>?): String {
        if (ruleNames != null) {
            if (t is RuleContext) {
                val ruleIndex = (t as RuleContext).getRuleContext().getRuleIndex()
                val ruleName = ruleNames.get(ruleIndex)
                val altNumber = (t as RuleContext).getAltNumber()
    ...
    fun _findAllNodes(t: ParseTree, index: Int, findTokens: Boolean,
        ...
        if (findTokens && t is TerminalNode) {
            ...
            if (tnode.symbol.getType() === index) nodes.add(t)
